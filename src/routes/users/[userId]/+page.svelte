<script lang="ts">
  ...
  import { formatJoinDate, getUserInitials } from "./helpers";
...
<div class="min-h-[calc(100vh-150px)] bg-background">
  ...
  <div class="max-w-3xl mx-auto">
    ...
    <div class="relative">
      ...
      <div class="absolute -bottom-10 right-4">
        ...
        {:else}
          ...
          {@render unauthenticatedUsersView()}
        {/if}
      </div>
    </div>
    ...
    <div class="px-4 pt-20 pb-4">
      {@render userProfileInfo({ userData: data })}
...
<!-- Current User View Snippet -->
{#snippet currentUserView()}
...
{#snippet userProfileInfo({ userData }: { userData: PageData })}
  <!-- User Name and Handle -->
  <div class="mb-3">
    <h1 class="text-2xl font-bold">{userData.user.name}</h1>
    <p class="text-muted-foreground">@{userData.user.name}</p>
  </div>
  ...
  <div class="flex items-center gap-4 mb-3 text-muted-foreground text-sm">
    ...
    <div class="flex items-center gap-1">
      ...
      <span>Joined {formatJoinDate(userData.user.createdAt)}</span>
    </div>
  </div>
  ...
  <div class="flex items-center gap-4 mb-4">
    <div class="flex items-center gap-1">
      <span class="font-bold">{userData.stats.followingCount}</span>
      <span class="text-muted-foreground">Following</span>
    </div>
    <div class="flex items-center gap-1">
      <span class="font-bold">{userData.stats.followersCount}</span>
      <span class="text-muted-foreground">Followers</span>
    </div>
  </div>
{/snippet}

<!-- Verification Notice Snippet -->
{#snippet verificationNotice()}
...
<!-- Unauthenticated Users View Snippet -->
{#snippet unauthenticatedUsersView()}
